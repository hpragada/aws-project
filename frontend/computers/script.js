...
function requireUser(){
  ...
  if(!user || !user.email) throw new Error('Please login on the home page before using cart checkout.');
  return user;
}

function userQuery(){
  return new URLSearchParams({ email: requireUser().email }).toString();
}

async function saveCartItem(id){
  const p = getById(id);
  if(!p || !cart[id]) return;
  try{
...
async function updateCartItem(id){
  await fetch(`${API_BASE}/cart/items`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: requireUser().email, product_id: id, quantity: cart[id] || 0 })
  });
}
...
async function deleteCartItem(id){
  try{
    await fetch(`${API_BASE}/cart/items`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: requireUser().email, product_id: id })
    });
  }catch(e){ console.warn('Cart delete failed', e); }
}

async function clearCartInDb(){
  try{
    await Promise.all(Object.keys(cart).map(id => deleteCartItem(id)));
...
async function loadCartFromDb(){
  try{
    ...
    const res = await fetch(`${API_BASE}/cart?${new URLSearchParams({ email: user.email })}`);
    const data = await res.json();
    if(!res.ok) throw new Error(data.error || 'Unable to load cart');
    cart = {};
    (data.items || []).forEach(item => {
      cart[item.product_id] = Number(item.quantity) || 1;
    });
    renderCart();
  }catch(e){
...
async function placeOrderInDb(method='UPI'){
  const user = requireUser();
  const payload = {
    email: user.email,
    shipping_name: (typeof getCustomerName === 'function' ? getCustomerName() : '') || user.full_name || user.username || 'Store Customer',
    shipping_address: user.address || 'Address not provided',
    shipping_phone: (typeof getCustomerMobile === 'function' ? getCustomerMobile() : '') || user.phone || '',
    payment_method: method,
    payment_status: 'paid',
    transaction_reference: `${method}-${Date.now()}`,
    payment_notes: `Paid via ${method}`
  };
...
function addToCart(id, qty=1){
  try{ requireUser(); }catch(e){ alert(e.message); return; }
  cart[id] = (cart[id]||0) + qty;
  renderCart();
  saveCartItem(id);
  if(window.gsap) gsap.fromTo('#cartCount',{scale:0.9,opacity:0.6},{scale:1,opacity:1,duration:.28});
}
function removeFromCart(id){ delete cart[id]; renderCart(); deleteCartItem(id); }
function changeQty(id, q){ if(q<=0) removeFromCart(id); else { cart[id]=q; renderCart(); updateCartItem(id).catch(e => console.warn('Cart update failed', e)); } }
function renderCart(){
  cartList.innerHTML = '';
...
window.addEventListener('load', () => {
  if(!getCurrentUser()){
    redirectHomeForLogin();
    return;
  }
  if(window.gsap) gsap.to(preloader,{opacity:0,duration:.6,onComplete:()=>preloader.style.display='none'});
  products = catalog[category] || [];
  renderGrid(products); renderCarousel(products); initHero(); renderCart(); loadCartFromDb(); revealOnScroll();
});
...
document.getElementById('emptyCart').onclick = ()=> { clearCartInDb(); cart = {}; renderCart(); }
